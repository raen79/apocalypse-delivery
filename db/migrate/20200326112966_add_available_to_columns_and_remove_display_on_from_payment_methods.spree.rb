...
class AddAvailableToColumnsAndRemoveDisplayOnFromPaymentMethods < ActiveRecord::Migration[
  5.0
]
  def up
    add_column(
  ...
  def down
    ...
    execute(
      'UPDATE spree_payment_methods ' \
        "SET display_on='' " \
        "WHERE (available_to_users=#{quoted_true} AND available_to_admin=#{
          quoted_true
        })"
    )
    execute(
      'UPDATE spree_payment_methods ' \
        ...
        "WHERE (available_to_users=#{quoted_true} AND NOT available_to_admin=#{
          quoted_true
        })"
    )
    execute(
      'UPDATE spree_payment_methods ' \
        ...
        "WHERE (available_to_admin=#{quoted_true} AND NOT available_to_users=#{
          quoted_true
        })"
    )
